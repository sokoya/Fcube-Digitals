...
import { GetEtisalatdata, BuyData, ClearErrorMgs } from "../../Redux/Actions/Data.action";
...
class EtisalatdataScreen extends Component {
    ...
    HandleBuydataplan  = async()=>{
      const {dataplan, success, errorMgs, loading} = this.props;
        ...
        await this.props.BuyData(this.state.userPhone,'9mobile',this.state.dataplanId,id)
        success? this.setState({showsuccess:true}):''
       // Alert.alert(this.state.dataplanId, this.state.userPhon
    };

    getdataplan= async()=>{  await this.props.GetEtisalatdata();}
    render(){
...
export default connect( MapStateToProps, {GetEtisalatdata,BuyData, ClearErrorMgs})(EtisalatdataScreen);
...
FormGroup:{ marginTop:10,margin:20 },

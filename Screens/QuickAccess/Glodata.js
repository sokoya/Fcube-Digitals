...
import { GetGlodata, BuyData, ClearErrorMgs } from "../../Redux/Actions/Data.action";
...
class GlodataScreen extends Component {
    ...
    HandleBuydataplan = async()=>{
      const {dataplan, success, errorMgs, loading} = this.props;
        ...
        await this.props.BuyData(this.state.userPhone,'glo',this.state.dataplanId,id)
        success? this.setState({showsuccess:true}):''
       // Alert.alert(this.state.dataplanId, this.state.userPhone)

    };

    getdataplan= async()=>{  await this.props.GetGlodata();}
    render(){
...
<Text style={{color:'red', textAlign:"center", fontSize:18}}>{errorMgs}</Text>
        ...
        >
          ...
          { dataplan.map((item, index)=>(<Picker.Item key={index}   fontSize={9} label ={  '\u20A6'+ item.amount  +" - " + item.plan_name   }  value={item.plan_id} />
...
export default connect( MapStateToProps, {GetGlodata,BuyData, ClearErrorMgs})(GlodataScreen);
...
FormGroup:{ marginTop:20 , margin:20,},

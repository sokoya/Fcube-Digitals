...
import { ValidatePh, clearLoginErrorMessage, PayPh} from "../../Redux/Actions/Electricity.action";
...
/////////////////////////////////////////////
        ...
        await this.props.ValidatePh(this.state.dataplanId, this.state.userPhone)
    };
    ...
    MakePayment= async()=>{  
      ...
      await this.props.PayPh(this.state.dataplanId, this.state.userPhone,this.state.Amount,id, product_code) }
    render(){
...
{ fsuccess?<View>
     ...
     //   disabled={this.state.Checked ? false: true}
       onPress={()=>this.MakePayment()}
           style={[Styles.MakePaymentBtn, {marginBottom:50}]}  >
            <Text style= {Styles.nextText}>
             Make Payment
            </Text>
          </TouchableOpacity>
    ...
    <AwesomeAlert style={{
      ...
      fontSize = {14}
      messageStyle= {{  color:'green', textAlign:'center', fontSize:14, padding:10}}
...
export default connect( MapStateToProps, {ValidatePh,PayPh, clearLoginErrorMessage })(PhScreen);
...
card:{ backgroundColor:'lightgrey', marginTop:10, padding:20, borderRadius:5}
});
